...
/***********************************************************
    ...
    1/16/2005      1.4     Moved the serial received ISR to
                           ...
                           written in assembly.
	4/17/2012				Updated for modern compilation
...
{	
	...
	UBRRH = 0x00;	/* set up the baud rate registers so the UART will operate at 115.2 Kbps */
#ifdef NO_CRYSTAL
	UBRRL = 18; 	/* 18 for double clocking at 115.2 kbps not used */
#else
	UBRRL = 0x08;  /* for 16 MHz crystal at 115.2 kbps */
#endif
	UCSRB = (1<<RXCIE)|(1<<RXEN)|(1<<TXEN);	/* enable the tx and rx capabilities of the UART...as well as the receive complete interrupt */
	UCSRC = (1<<UCSZ1)|(1<<UCSZ0); /* set up the control registers so the UART works at 8N1 */

#ifdef NO_CRYSTAL     
    UCSRA = (1<<U2X);    /* set the baud rate to use the double-speed not used*/
#endif    
