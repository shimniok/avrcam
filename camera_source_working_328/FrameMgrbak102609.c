...
static unsigned int guard1; // to guard ptr below when clearing trackedObjectTable
static unsigned int guard2; // 
...
{
// assume for the moment that sizeof returns the size in ints ... jcd 10/21/09
	memset(trackedObjectTable,0x00,sizeof(trackedObjectTable));
	//memset(trackedObjectTable,0x00,64);
...
void FrameMgr_acquireFrame(void)
{
    ...
    //    memset(trackedObjectTable,0x00,64); // 10/21/09 jcd 
